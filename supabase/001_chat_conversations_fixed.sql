...
-- Supabase SQLエディタで実行してください（フェーズ7対応版）
-- ==========================================
...
-- ==========================================
-- エージェント切り替え機能用ビューと関数（フェーズ7対応）
-- ==========================================

-- 会話詳細ビュー（ConversationWithDetailsに対応）
...
ORDER BY COALESCE(
    (SELECT m2.created_at FROM public.messages m2 WHERE m2.conversation_id = c.id ORDER BY m2.created_at DESC LIMIT 1), 
    c.created_at
) DESC;
...
COMMENT ON VIEW public.conversations_with_details IS 'フェーズ7対応: 会話詳細情報（最後のエージェント含む）'; 